...
- [ ] P149 Windows поставляется одним GUI-установщиком без терминального окна и
  без пользовательской пары сырых EXE (`DUD-UI-180`)
- [ ] P150 Linux получает полноценную Flutter GUI-сборку и устанавливаемый
  пакет; TUI остаётся дополнительным (`DUD-UI-180`)
- [ ] P151 На macOS, Windows и Linux работает настраиваемый пользовательский
  автозапуск, стартующий скрыто (`DUD-UI-181`)
- [ ] P152 Крестик скрывает окно в трей; трей открывает окно и явно завершает
  Дудку (`DUD-UI-181`)
- [ ] P153 Иконка Dock/taskbar/launcher показывает число новых чужих сообщений
