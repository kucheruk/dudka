...
- [ ] P110 Спека MCP home-agents Accepted: surface tools, no-WAN, формат префикса ника `{agent}·{model}` (`DUD-AGT-101/110`)
- [ ] P111 Advertise/discovery агента в LAN (peer + agent marker; тот же no-WAN контур)
- [ ] P112 Нормализация/валидация ника агента с обязательным префиксом; человек без префикса
- [ ] P113 MCP tool: agent → чат (send текста в общий feed)
- [ ] P114 MCP tool: чат → agent (получить входящий текст)
- [ ] P115 Smoke: два процесса (человек + agent stub) на LAN/loopback обмениваются текстом; префикс виден в ленте
