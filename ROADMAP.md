...
## Фаза 10 — Исправление выбранной иконки

- [x] P146 Установить точно выбранный владельцем четырёхблочный знак во все GUI-сборки (`DUD-UI-172`)

---

## Фаза 11 — Честные файлы и online

- [x] P147 Скачанный файл получает исходное имя и расширение; внутренний blob не показывается пользователю (`DUD-FILE-140`)
- [x] P148 Текущий пользователь виден в «СОСЕДЯХ» и входит в `онлайн N` (`DUD-UI-120`)

---

## Фаза 12 — Равноправный desktop

- [x] P149 Windows получил единый GUI-пакет без терминального окна и
  пользовательской пары сырых EXE; установщик позже заменён P156
  (`DUD-UI-180`)
- [x] P150 Историческая Linux GUI-сборка заменена в P157 основным TUI
  (`DUD-UI-180`)
- [x] P151 На macOS, Windows и Linux работает настраиваемый пользовательский
  автозапуск, стартующий скрыто (`DUD-UI-181`)
- [x] P152 Крестик скрывает окно в трей; трей открывает окно и явно завершает
  Дудку (`DUD-UI-181`)
- [x] P153 Иконка Dock/taskbar/launcher показывает число новых чужих сообщений
  и очищается при открытии окна (`DUD-UI-182`)

---

## Фаза 13 — Сохранение данных и быстрая установка Linux

- [x] P154 Ник и локальный хвост 200 сообщений переживают замену приложения и
  повторный старт движка (`DUD-CHAT-122`)
- [x] P155 Исторический DEB-однострочник заменён безопасным TUI installer в P157
  (`DUD-UI-183`)

## Фаза 14 — Portable Windows

- [x] P156 Windows поставляется только переносимым ZIP без установщика; в корне
  архива один пользовательский `dudka.exe`, движок скрыт в `internal/`,
  нативный CI распаковывает и запускает GUI (`DUD-UI-180`)

---

## Фаза 15 — Linux terminal-first

- [x] P157 Linux-поставка заменена на выразительный TUI + engine; однострочник
  ставит два статических бинаря без root, apt и GUI-зависимостей (`DUD-UI-183`)
- [x] P158 macOS и Linux используют общий discovery; TUI ищет через `/search`,
  не перехватывает `S` и отделяет ошибки от подсказок (`DUD-UI-120`)
- [x] P159 Linux TUI после ошибки показывает F5 и копирует полный технический
  текст в буфер терминала без системных зависимостей (`DUD-UI-121`)
- [x] P160 macOS release build не содержит предупреждения Xcode о Flutter
  Assemble без выходных файлов

---

## Фаза 16 — Диагностика Linux TUI

- [x] P161 F5 копирует готовый пакет для агента: контекст ошибки и ограниченный
  хвост `tui.log`, без сообщений и идентификаторов соседей (`DUD-UI-121`)

---

## Фаза 17 — WebRTC без домашнего сервера

- [x] P162 Информированное согласие до первого signaling-запроса
  (`DUD-WEB-101`)
- [x] P163 Сигнальный hub Студии: IP-rendezvous, память-only, лимиты и
  same-origin защита (`DUD-WEB-110`, `DUD-WEB-140`)
- [x] P164 Две вкладки автоматически строят WebRTC mesh, показывают online и
  обмениваются текстом/хвостом (`DUD-WEB-120`)
- [x] P165 Файлы остаются черновиком до отправки и идут чанками напрямую по
  WebRTC (`DUD-WEB-130`)
...
- [x] P171 Обе стороны WebRTC показывают живой таймер согласования и
  гарантированно завершают его ошибкой (`DUD-WEB-110`)
- [x] P172 `ICE failed` немедленно завершает индикатор и сохраняет понятную
  ошибку прямого маршрута (`DUD-WEB-110`)
- [x] P173 Safari получает прямой LAN-маршрут через явно объяснённое
  разрешение, а signaling гарантирует порядок trickle ICE (`DUD-WEB-110`)
...
регистрация, доска в «Делах» как бэклог, callhome/telemetry. WAN-исключения:
desktop update по `DUD-UPD-*` и разрешённый браузерный signaling по
`DUD-WEB-*`.
