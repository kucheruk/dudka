...
- **Linux TUI:** та же топология в тексте (peers | feed | input).
...
- **Compose:** поле ввода + кнопка «ДУНУТЬ» (primary) + attach (ghost).
...
- Исходящий интернет в UI (update badges, «войти через…»).
