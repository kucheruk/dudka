...
Engine в рантайме не устанавливает исходящие соединения к адресам вне link-local, loopback и частных диапазонов IPv4/IPv6 (RFC1918, Unique Local). Попытка конфигурации «облачного» endpoint игнорируется или отвергается.

Проверка:

- интеграционный тест: блокировщик/прокси фиксирует ноль WAN-попыток при типичном сценарии чата; *(полный сценарий чата — позже; P025 закрывает негатив с фейковым dialer)*
