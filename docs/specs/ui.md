...
запускает системный пакетный менеджер, не просит `sudo`, не меняет firewall и
не открывает старые UDP/TCP-порты. Основная Linux-проверка на Ubuntu CI
дополнительно доказывает, что список системных пакетов не изменился.

Проверка:

- `curl -fsSL https://zamoo.team/dudka/install.sh | sh` устанавливает TUI;
- неверная контрольная сумма останавливает установку;
- список пакетов и правила firewall до и после установки совпадают;
...
Desktop-шапка всегда показывает малозаметную текущую версию приложения.
После разрешения интернет-доступа рядом виден честный статус signaling:
«сигнал есть» при открытом WebSocket или «сигнал ждём» во время подключения.
Потеря GUI-процесса завершает запущенный им `dudkad` не позже чем через две
секунды, даже если штатный обработчик выхода не выполнился.

Проверка:

- widget test видит `ДУДКА vX.Y.Z` и оба состояния signaling;
- bundled engine получает PID родителя и завершает работу после его потери;
- закрытые старые GUI не оставляют отдельные фоновые экземпляры engine.

Зависимости: DUD-NET-170, DUD-UI-181
ADR: не требуется
