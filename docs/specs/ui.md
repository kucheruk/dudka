...
запускает системный пакетный менеджер и не требует root. Основная Linux-проверка
на Ubuntu CI дополнительно доказывает, что список системных пакетов не изменился.
