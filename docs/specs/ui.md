...
Пользовательская desktop-поставка на macOS, Windows и Linux открывает один и
тот же Flutter GUI. Windows публикуется одним установщиком `.exe`: он создаёт
один пользовательский ярлык Дудки и не показывает терминальное окно.
`dudkad` устанавливается как внутренний компонент и самостоятельно
пользователю не предлагается. Linux публикуется как устанавливаемый GUI-пакет;
TUI остаётся отдельным дополнительным инструментом.

Проверка:

- Windows installer → меню «Пуск» → графическое окно Дудки без консоли;
- в пользовательском блоке загрузки нет пары сырых `dudka.exe`/`dudkad.exe`;
- Linux GUI открывает тот же чат, настройки и файловый интерфейс;
