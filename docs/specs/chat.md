...
Устройство сохраняет локальный хвост последних 200 сообщений и выбранный ник
в пользовательском каталоге данных вне app bundle и каталога установки.
Замена приложения, автообновление и повторная установка поверх текущей версии
не меняют ник, `peer_id` и локальный хвост. Полное удаление пользовательских
данных остаётся отдельным явным действием.

Сборки GUI до 0.5.0 могли принудительно заменить ник на `ДУДКА`. При обнаружении
этого старого служебного значения GUI один раз повторно просит ник и сохраняет
его независимо от файла движка.

Проверка:

- сообщение и ник доступны после остановки движка, замены бинаря и повторного
  старта с тем же каталогом данных;
- локальный файл истории содержит не более 200 сообщений;
- GUI не передаёт `-name ДУДКА` при обычном старте;
- evidence: `internal/chat/store_ring_test.go`,
  `apps/dudka/test/engine_host_test.dart`,
  `apps/dudka/test/first_run_test.dart`,
  `./scripts/reinstall_persistence_test.sh`.

Зависимости: DUD-CHAT-110, DUD-CHAT-120
ADR: не требуется
