...
static void desktop_method_call_cb(FlMethodChannel* channel,
  ...
  if (g_strcmp0(method, "setBadge") != 0) {
  ...
  gint64 count =
      fl_value_get_type(args) == FL_VALUE_TYPE_INT ? fl_value_get_int(args) : 0;
  count = MAX(count, 0);
  ...
  if (bus != nullptr) {
    ...
    g_dbus_connection_emit_signal(
        ...
        g_variant_new("(sa{sv})", "application://team.zamoo.dudka.desktop",
                      &properties),
        nullptr);
  }
...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "ДУДКА");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "ДУДКА");
  }
  ...
  self->desktop_channel = fl_method_channel_new(
      ...
      "team.zamoo.dudka/desktop", FL_METHOD_CODEC(codec));
